...
// Endpoint 7: Log Monitor and Task List
async function getLatestTaskFolder() {
  try {
    ...
    for (const conv of conversations) {
      ...
      if (stat.isDirectory() && stat.mtimeMs > latestTime) {
        const tasksPath = path.join(convPath, '.system_generated', 'tasks');
        try {
          const tStat = await fs.stat(tasksPath);
          if (tStat.isDirectory()) {
            latestTime = stat.mtimeMs;
            latestFolder = tasksPath;
          }
        } catch (e) {}
      }
    }
    ...
    console.error('Error finding latest brain task folder:', err);
    return null;
  }
}
...
app.get('/api/tasks', async (req, res) => {
  try {
    const tasksDir = await getLatestTaskFolder();
    if (!tasksDir) {
      return res.json([]);
    }
...
app.get('/api/tasks/:id/log', async (req, res) => {
  try {
    ...
    const logPath = path.join(tasksDir, `${req.params.id}.log`);
    try {
